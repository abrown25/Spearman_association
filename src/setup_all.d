...
T[] setup(T)(ref File[3] fileArray, Opts opts)
{
    ...
    // no header line then write your own
        ...
        headerLine ~= "".reduce!((a, b) => a ~ "F" ~ to!string(b + 1) ~ "\t")(iota(
            0, opts.skip));

    headerLine ~= "Cor\tT_stat\tP";

    if (opts.run)
        headerLine ~= opts.pval ? "\tPermP" : opts.min ? "\tPermP\tFWER" : opts.fdr ? "\tPermP\tFDR" : "".reduce!((a,
            b) => a ~ "\tP" ~ to!string(b + 1))(iota(0, opts.number));
    ...
    if (opts.match)
    {
        ...
        {
            auto genIndNotPhen = zip(orderPhen, genId).filter!(a => a[0] == -1).map!(
                a => a[1]).array;
            if (genIndNotPhen.length == 1)
                stderr.writeln("Failed to run analysis: individual ", genIndNotPhen[0],
                ...
                stderr.writeln("Failed to run analysis: individuals ",
                    genIndNotPhen[0 .. ($ - 1)].join(", "), " and ", genIndNotPhen[
                    $ - 1],
