...
string readGenotype(string x)
{
  ...
  if (opts.ttest)
    ...
    transform(rank(rankGenotype, orderBuffer));
";
...
void noPerm(T)(ref File[3] fileArray, in Opts opts, in T[] rankPhenotype)
{
  ...
  auto orderBuffer = new size_t[nInd];
...
void simplePerm(T)(ref File[3] fileArray, in Opts opts, in T[] rankPhenotype)
{
  ...
  const T[] perms = getPerm(opts, rankPhenotype);
  ...
  foreach (line; fileArray[F.gen].byLine)
  {
    try
    {
      ...
      fileArray[F.out_].writeln(chunks(perms,
        nInd).map!(a => to!string(corPvalue(dotProduct(rankGenotype, a), nInd))).join("\t"));
    }
    catch (VarianceException e)
    {
      fileArray[F.out_].writeln(varErr);
...
void minPerm(T)(ref File[3] fileArray, in Opts opts, in T[] rankPhenotype)
{
  ...
  foreach (line; fileArray[F.gen].byLine)
  {
    try
    {
      ...
      auto simplePerm = map!(a => to!T(fabs(dotProduct(rankGenotype, a))))(chunks(perms,
        nInd)).array;
