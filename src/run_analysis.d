...
void simplePerm(T)(ref File[3] fileArray, in Opts opts, immutable(T[]) rankPhenotype)
{
    ...
    foreach (line; fileArray[F.gen].byLine)
    {
        try
        {
            ...
            fileArray[F.out_].writeln(chunks(perms,
                nInd).map!(a => to!string(corPvalue!(T)(dotProduct(rankGenotype, a),
                nInd))).join("\t"));
        }
        catch (VarianceException e)
        {
...
void pvalPerm(T)(ref File[3] fileArray, in Opts opts, immutable(T[]) rankPhenotype)
{
    ...
    foreach (line; fileArray[F.gen].byLine)
    {
        try
        {
            ...
            fileArray[F.out_].writeln("\t", 1.0 * chunks(perms,
                nInd).map!(a => fabs(dotProduct(rankGenotype, a))).count!(a => a > corReal) / nPerm);
        }
        catch (VarianceException e)
        {
            fileArray[F.out_].writeln(varErr);
...
void minPerm(T)(ref File[3] fileArray, in Opts opts, immutable(T[]) rankPhenotype)
{
    ...
    foreach (line; fileArray[F.gen].byLine)
    {
        try
        {
            ...
            auto simplePerm = map!(a => to!T(fabs(dotProduct(rankGenotype, a))))(
                chunks(perms, nInd)).array;
            tmpFile.writeln("\t", 1.0 * simplePerm.count!(a => a > corReal) / nPerm);
    ...
    foreach (line; tmpFile.byLine)
    {
        ...
        else
        {
            ...
            fileArray[F.out_].writefln("%g\t%s\t%g", corStat,
                join(splitLine.drop(corCol + 1), "\t"), adjusted);
        }
    }
}
...
void fdrCalc(T)(ref File[3] fileArray, in Opts opts, immutable(T[]) rankPhenotype)
{
    ...
    foreach (e; 1 .. orderIndex.length)
        adjusted[orderIndex[e]] = sortPerm[0 .. (sortPerm.length - cast(
            size_t) adjusted[orderIndex[e - 1]])].upperBound(fabs(realCor[orderIndex[
            e
        ]]) - EPSILON).length.to!T + adjusted[orderIndex[e - 1]];
    ...
    {
        dupcount++;
        if (i == orderIndex.length - 1 || fabs(realCor[e]) - EPSILON > fabs(
                realCor[orderIndex[i + 1]]))
        {
