...
unittest
{
    ...
    double[] residualsFromR = [
        -1.00027816411683, -1.72461752433936, -0.275938803894297,
        1.51766342141864, 1.48317107093185
    ];
...
void covariates(string covF, ref double[] phen)
{
    ...
    foreach (line; covFile.byLine)
    {
        ...
        enforce(splitLine.length == nCov, new InputException(
            "Failed to run analysis, covariate measurements missing"));
        covOut ~= to!(double[])(splitLine);
        nInd++;
    }

    enforce(nInd == phen.length, new InputException(
        "Failed to run analysis, covariates file does not have the same number of individuals as phenotype file"));
...
unittest
{
    ...
    double[] residualsFromR = [
        -0.0988744898987968, 0.32101118013441, 0.598800504240901, 0.798569051911944,
        0.00240916094127165, -0.512816562906186, -0.586676086840575,
        -0.690613979610441, -0.801131138946856, 0.969322360974328
    ];
...
pure nothrow T[3] correlation(T)(in T[] vector1, immutable(T[]) vector2)
{
    ...
    results[1] = results[0] * sqrt((vector1.length - 2) / (1 - results[0] * results[
        0
    ]));
...
T[] getPerm(T)(in Opts permOpts, immutable(T[]) vector)
{
    ...
    T[] outPerm = vector.dup.cycle.take(permOpts.number * vector.length).array;
...
unittest
{
    ...
    double[] permPvalsR = [
        0.295293228837933, 0.620081751733492, 0.248565693244932, 0.523263392865814
    ];
