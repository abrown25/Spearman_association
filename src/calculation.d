...
void covariates(string covF, ref double[] phen)
{
  ...
  foreach (line; covFile.byLine)
  {
    ...
    enforce(splitLine.length == nCov,
      new InputException("Failed to run analysis, covariate measurements missing"));
    covOut ~= to!(double[])(splitLine);
    nInd++;
  }

  enforce(nInd == phen.length,
    new InputException(
...
unittest
{
  ...
  double[] residualsFromR = [
    -0.0988744898987968, 0.32101118013441, 0.598800504240901,
    0.798569051911944, 0.00240916094127165, -0.512816562906186,
    -0.586676086840575, -0.690613979610441, -0.801131138946856, 0.969322360974328
  ];
...
unittest
{
  ...
  double[] permPvalsR = [0.295293228837933, 0.620081751733492,
