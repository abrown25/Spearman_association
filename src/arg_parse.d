...
class Opts
{
    ...
    this(string[] args)
    {
        try
        {
            getopt(args, "pheno|p", &phenotype, "geno|g", &genotype, "out|o",
                ...
                "geno-skip|gs", &skip, "perm", &getPerms, "pheno-id|pid",
                &pid, "geno-id|gid", &gid, "fwer", &min, "pval", &pval, "fdr",
                &fdr, "pi", &getPi, "ttest", &ttest, "nocheck", &nocheck,
                "match", &match, "help", &help, "version", &version_);
        }
        catch (Exception e)
        {
            writeln(e.msg);
    ...
    private void checkOptions()
    {
        ...
        {
            stderr.writeln(
                "Failed to run analysis: Permutations must be specified with the -perm flag");
            exit(0);
        }
    ...
    private void getPi(string opt, string val)
    {
        try
        {
            ...
            enforce(num <= 1 && num > 0, new InputException(
                "Failed to run analysis: π₀ value must lie in (0, 1]"));
        }
        catch (ConvException e)
        {
            stderr.writeln("Failed to run analysis: Non-numeric argument to -pheno-col");
